...
* @param  {XMLElement} parentNode xmlbuilder node which will receive new nodes
* @param  {object}     input      Complex object with specific shape to be converted to YML object
*                                 @link lib/schemas.js/Collections
* @return {XMLElement} `parentNode` with newly created nodes based on input
*/
const createCollection = (parentNode, input) =>
  forNode(parentNode.node('collection'), input, (collection, key, entry) => {
    switch (key) {
      // Those should be attributes according to specs
      case 'collectionId':
      case 'name':
      case 'url':
        collection.att(key, entry)
        break

      case 'picture':
        entry.forEach((e) => collection.node(key).txt(e))
        break

      case 'description':
        collection.node(key).dat(entry)
        break

      default: collection.node(key).txt(entry)
    }
  })
...
const createYML = (input, options) => {
  ...
  forNode(yml, input, (yml, key, entry) => {
    switch (key) {
      ...
      case 'collections':
        forNodeEach(yml.node(key), entry, (collections, entry) => {
          createCollection(collections, entry)
        })
        break
